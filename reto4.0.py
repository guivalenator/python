...
def cercana(lat1, lon1):
    ...
    ordenada=sorted(puntos, key=lambda x: x[2])
    conexiones=[]
    for i in range(2):
        conexiones.append([])
        for j in range(4):
    ...
    final=sorted(conexiones, key=lambda x: x[3])
    
    print(final)
    print("Zonas wifi cercanas con menos usuario")
    print(f"La zona wifi {i+0}: ubicada en {final[0][0]},{final[0][1]} a {final[0][2]} metros, tiene en promedio {final[0][3]} usuarios")
    print(f"La zona wifi {i+1}: ubicada en {final[1][0]},{final[1][1]} a {final[1][2]} metros, tiene en promedio {final[1][3]} usuarios")
    print("\n")
